...
sub itk_detect {
	my ($model) = @_;
	if ( $model  =~ /(.*)itk(.*)/)  {
		show_crit_box(); print "MPM ITK was detected, easyapachebuddy.pl does odd things so we quit. Sorry.\n";
...
# THIS IS NOT A GOOD WAY TO DETERMINE UPTIME OF APACHE!!! 
# THIS ONLY GETS THE UPTIME OF THE SERVER (not of Apache)... That's fine unless you're on a VM which might be suspended.
# Using systemctl status would be better.
# But of course, CloudLinux 6 is still supported and has no systemctl. There's no other way other than the original ps method
# to obtain the Apache uptime. 
...
sub get_apache_uptime {
	...
	if ($major_redhat_version lt 7 ) {
		...
		@apache_uptime = ( $days, $hours, $minutes, $seconds );
	}
	else {
    	my $now = localtime(time());
    	my $epoch=time();
    	my ($date,$time) = (split /\s+/, qx[ systemctl status httpd | grep -i active ] )[6,7];
    	my ( $year, $mon, $day ) = split /-/, $date;
    	my ( $hh, $mm, $ss ) = split /:/, $time;
    	my $current = qx[ date "+%m/%d/%Y %H:%M:%S" ];
    	chomp($current);
    	my $date1 = "$mon/$day/$year $hh:$mm:$ss";
    	my $date2 = "$current";
    	my @times = map Time::Piece->strptime(/(\d.+)/, '%m/%d/%Y %H:%M:%S'), $date1, $date2;
    	my $delta = $times[1] - $times[0];
    	my ( $days, $hours, $minutes, $seconds ) = 0;
    	my $cnt=0;
    	( $days ) = (split( /,/, $delta->pretty))[$cnt] if ( $delta->pretty =~ m/day/ );
    	$cnt++ if ( $days );
    	( $hours ) = (split( /,/, $delta->pretty))[$cnt] if ( $delta->pretty =~ m/hour/ );
    	$cnt++ if ( $hours );
    	( $minutes ) = (split( /,/, $delta->pretty))[$cnt] if ( $delta->pretty =~ m/minute/ );
    	$cnt++ if ( $minutes );
    	( $seconds ) = (split( /,/, $delta->pretty))[$cnt] if ( $delta->pretty =~ m/second/ );
    	$days =~ s/day//g;
    	$days =~ s/days//g;
    	$days =~ s/ s//g;
    	push @apache_uptime, $days;
    	$hours =~ s/hour//g;
    	$hours =~ s/hours//g;
    	$hours =~ s/ s//g;
    	push @apache_uptime, $hours;
    	$minutes =~ s/minute//g;
    	$minutes =~ s/minutes//g;
    	$minutes =~ s/ s//g;
    	push @apache_uptime, $minutes;
    	$seconds =~ s/second//g;
    	$seconds =~ s/seconds//g;
    	$seconds =~ s/ s//g;
    	push @apache_uptime, $seconds;
	}
...
sub get_php_setting {
	...
	# code to address bug raised in issue #197 (cli memory limits on ubuntu)
...
sub generate_standard_report {
	...
	if ($model eq "prefork") {
		...
		# make a logfile entry at /var/log/easyapachebuddy.log
		open (LOGFILE, ">>/var/log/easyapachebuddy.log");
		if ( our $apache_version =~ m/.*\s*\/2.4.*/) {
	...
	if ( $phpfpm_detected ) {
		insert_hrule();
		print "${BLUE}PHP-FPM DETECTED:${ENDC}\n";
        my $DefaultPHPraw = get_whmapi1( 'php_get_system_default_version');
        my $phpDefault = $DefaultPHPraw->{data}->{version}; 
        show_info_box();
        print "${YELLOW}Displaying contents of /opt/cpanel/$phpDefault/root/etc/php-fpm.conf file [DEFAULT]\n${ENDC}";
        open (my $fh, "<", "/opt/cpanel/$phpDefault/root/etc/php-fpm.conf");
        while ( <$fh> ) {
            chomp($_);
            next if( $_ =~ m/^($|;|\[global\])/ );
            print "\t\\_ $_\n";
        ...
        close($fh);
        if (@PHPFPMUSERS) {
            splice(@PHPFPMUSERS, 0,1);
            #print "\nIndividual users php-fpm.conf data:\n";
            print "\n";
            foreach my $user(@PHPFPMUSERS) {
                chomp($user);
                my $userDomain = get_domain_for( $user );
                if ( $userDomain ) {
                    my $userPHPver = get_php_version_for( $userDomain );
                    show_info_box();
                    print "Getting php-fpm.conf data for $user [ $userDomain ] [ PHP Version: $userPHPver ]\n";
                    open (my $fh, "<", "/opt/cpanel/$userPHPver/root/etc/php-fpm.d/$userDomain.conf");
                    while ( <$fh> ) {
                        chomp($_);
                        next if( $_ =~ m/^($|;|\[global\])/ );
                        print "${GREEN}\t\\_ $_\n${ENDC}";
	...
	insert_hrule();
	if ( ! $NOINFO ) { 
		print "A log file entry has been made in: /var/log/easyapachebuddy.log for future reference.\n\n";
		print "Last 5 entries:\n\n";
		my $entries = `tail -5 /var/log/easyapachebuddy.log`;
		print $entries."\n";
	}
}
...
sub print_header {
	...
	if ( ! $NOHEADER ) {
		my $headerstring = "easyapachebuddy.pl report for $servername ($ipaddr)";
...
sub show_important_message {
	if ( ! $NOINFO ) {
		print "\n${RED}** IMPORTANT MESSAGE **\n\neasyapachebuddy is not a troubleshooting tool. Do not use it to try and determine why your site went\ndown or why it was slow.\n\nPerform some proper investigations first, and only if you found that you were hitting the\nMaxRequestWorkers limit, or if your server was running out of memory\n(primarily due to excessive memory usage by Apache), should you run this script and refer to its output..\n${ENDC}\n";
	}
}

sub get_domain_for {
    my $lcUser = shift;
    my $getDomainRaw = get_whmapi1( 'accountsummary', "user=$lcUser" );
    my $domain = $getDomainRaw->{data}->{acct}->[0]->{domain};
    return $domain unless( not defined $domain );
    show_crit_box();
    print "Could not get domain name for $lcUser\n";
    return "";
}

sub get_php_version_for {
    my $tcDomain = shift;
    my $php_version_data = get_whmapi1('php_get_vhost_versions');
    for my $phpVhosts ( @{ $php_version_data->{data}->{versions} } ) {
        if ( $phpVhosts->{vhost} eq $tcDomain ) {
            if ( defined( $phpVhosts->{phpversion_source}->{system_default} ) ) {
                return "inherit";
            }
            else {
                return $phpVhosts->{version};
            }
        }
    }
}
...
sub preflight_checks {
    ...
    # Display default PHP version and multiphp versions installed
    our $MULTIPHP = 0;
    my $DefaultPHPraw = get_whmapi1( 'php_get_system_default_version');
    my $phpDefault = $DefaultPHPraw->{data}->{version};
    show_ok_box(); print "Default PHP Version: $phpDefault\n";
    my $mphp_versraw = get_whmapi1( 'php_get_installed_versions' );
    show_ok_box(); print "MultiPHP Versions Installed:\n";
    for  my $mphp_version ( @{ $mphp_versraw->{data}->{versions} } ) {
        print "\t\\_ " . $mphp_version . "\n";
    }
    our $MULTIPHP = 1;
	...
	unless ($apache_user_config eq "apache" or $apache_user_config eq "www-data" or $apache_user_config eq "nobody") {
	...
	our $maxclients;
	our $current_proc_count = `ps aux | egrep "httpd|apache2" | grep -v rotatelogs | grep -v easyapachebuddy | grep -v grep | wc -l`;
...
sub detect_package_updates {
	...
	if (ucfirst($distro) eq "Ubuntu" ) {
        $package_update = `apt-get update 2>&1 >/dev/null && dpkg --get-selections | xargs apt-cache policy | grep -1 Installed | sed -r 's/(:|Installed: |Candidate: )//' | uniq -u | tac | sed '/--/I,+1 d' | tac | sed -n 1~2p | egrep "^(ea-apache24|ea-php)"`;
	} else {
		$package_update = `yum check-update | egrep "^(ea-apache24|ea-php)"`;
	}
	if ($package_update) {
		if ( $package_update !~ /Failed|Error/) {
			if ( ! $NOWARN ) {
				show_crit_box(); print "${RED}There are pending package updates available for ea-apache24 and ea-php.${ENDC}\n";
...
sub detect_php_fatal_errors {
    ...
    my $DefaultPHPraw = get_whmapi1( 'php_get_system_default_version');
    my $phpDefault = $DefaultPHPraw->{data}->{version}; 
		our $SCANDIR = "/opt/cpanel/$phpDefault/root/usr/var/log/php-fpm/error.log";
		our %logfile_counts;
		grep_php_fatal($SCANDIR);
	}
	our %logfile_counts;
...
sub detect_maxclients_hits {
	...
	if ($process_name eq "/usr/sbin/httpd") {
		...
		our $maxclients_hits = `grep -i reached /var/log/apache2/error_log | egrep -v "mod" | tail -5`;
	} elsif ($process_name eq "/opt/rh/httpd24/root/usr/sbin/httpd") {
